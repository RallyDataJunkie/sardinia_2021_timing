...
```{r fsd_{{result_group}}_{{stage_code}}, cache=TRUE, results=FALSE, message=FALSE, warning=FALSE}
#library(formattable)
...
```{r test_pace_chart_{{result_group}}_{{stage_code}}, cache=TRUE}
...
section_pace_long_rebased %>%
    pace_map( xstart='start_dist', id_col='code',
         ...
         label_size=2) + xlab("Accumulated stagedistance (km)") + ylim(1.5, -1.5) + labs(title = paste0("Pace chart over splits, ", stage_code),
...
```{r test_ults_{{result_group}}_{{stage_code}}, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r ult_act_table_{{result_group}}_{{stage_code}}, cache=TRUE}
...
```{r test_uas_t_{{result_group}}_{{stage_code}}, cache=TRUE,}
...
```{r test_split_pace_chart_{{result_group}}_{{stage_code}}, cache=TRUE, warning=FALSE, error=FALSE, message=FALSE}
