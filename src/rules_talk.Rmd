...
rule_multi_results = multi_overall_results %>% map_driver_names(cars) %>% map_stage_codes_col(stages)
...
with ruleset('rule_multi_results'):
    ...
    def whos_in_first(c):
        ...
        txts.append(f'{c.m.code} was in first with a time of {c.m.totalTime}.')
    ...
    def whos_where(c):
        ...
        t = pickone_equally([f'with a time of {c.m.totalTime}',
                             f'{sometimes(f"{display_time(c.m.diffFirstS)} behind {pickone_equally(first_opts)}")}'],
                           prefix=', ')
...
```{python}
txts=[]
tmp = json.loads(pd_df.iloc[0].to_json())
post('rule_multi_results',tmp)
''.join(txts)
```
